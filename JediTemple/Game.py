...
class Game(ctk.CTkFrame):
    def __init__(self, master, rows = 15, columns = 15, difficulty = 'easy', **kwargs):
        super().__init__(master, **kwargs)

        # Core components
        self.board = Board(self, rows, columns, click_callback=self.handle_cell_click)
        self.player_manager = PlayerManager(self.board, 15)
        self.treasure_manager = TreasureManager(self.board, 5)
        self.dice = Dice()
